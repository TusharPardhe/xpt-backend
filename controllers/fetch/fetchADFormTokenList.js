...
const fetchADFormTokenList = async (request, response) => {
    ...
    try {
        const { userName, networkServer } = request.body;
        ...
        const xrpl_currency_list = Object.keys(currencies).map((a) => ({
            key: a,
            value: currencies[a].hex,
            text: a,
            limit: currencies[a].limit,
            issuer: currencies[a].issuer,
        }));
